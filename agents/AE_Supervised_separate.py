...
from agents.network.ae_supervised_actor_network import AE_Supervised_Actor_Network
from agents.network.ae_supervised_expert_network import AE_Supervised_Expert_Network
...
class AE_Supervised_separate_Network(object):
    def __init__(self, state_dim, state_min, state_max, action_dim, action_min, action_max, use_external_exploration,
        ...
        with self.graph.as_default():
            ...
            self.actor_network = AE_Supervised_Actor_Network(self.sess, self.input_norm, actor_layer_dim,
                                                             state_dim, state_min, state_max,
                                                             action_dim, action_min, action_max, config.actor_lr,
                                                             config.tau, config.num_modal, config.action_selection,
                                                             norm_type=self.norm_type)

            self.expert_network = AE_Supervised_Expert_Network(self.sess, self.input_norm, expert_layer_dim,
                                                               state_dim, state_min, state_max,
                                                               action_dim, action_min, action_max, config.expert_lr,
    ...
    def take_action(self, state, is_train, is_start):
        ...
        if is_train:
            ...
            if self.use_external_exploration:
                ...
                action_init = self.actor_network.sample_action(np.expand_dims(state, 0), False)[0][0]  # single batch, and single sample
    ...
    def update_network(self, state_batch, action_batch, next_state_batch, reward_batch, gamma_batch):
        ...
        self.actor_network.train_actor(state_batch, action_batch_final)
...
class AE_Supervised_separate(BaseAgent):
    def __init__(self, env, config, random_seed):
        super(AE_Supervised_separate, self).__init__(env, config)

        np.random.seed(random_seed)
        random.seed(random_seed)

        # Network
        self.network = AE_Supervised_separate_Network(self.state_dim, self.state_min, self.state_max,
