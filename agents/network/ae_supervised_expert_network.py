...
class AE_Supervised_Expert_Network(BaseNetwork):
    def __init__(self, sess, input_norm, layer_dim, state_dim, state_min, state_max, action_dim, action_min, action_max,
                 expert_lr, tau, norm_type):
        super(AE_Supervised_Expert_Network, self).__init__(sess, state_dim, action_dim, expert_lr, tau)
        ...
        # Supervised Expert network
        self.inputs, self.phase, self.action, self.q_prediction = self.build_network(scope_name='supervised_expert')
        self.net_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='supervised_expert')

        # Target network
        self.target_inputs, self.target_phase, self.target_action, self.target_q_prediction = self.build_network(scope_name='target_supervised_expert')
        self.target_net_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='target_supervised_expert')
    ...
    def network(self, inputs, action, phase):
        # shared net
        q_net = tf.contrib.layers.fully_connected(inputs, self.l1, activation_fn=None,
        ...
        # Q branch
        q_net = tf.contrib.layers.fully_connected(tf.concat([q_net, action], 1), self.l2,
                                                  activation_fn=None,
                                                  weights_initializer=tf.contrib.layers.variance_scaling_initializer(
                                                      factor=1.0, mode="FAN_IN", uniform=True),
                                                  # tf.truncated_normal_initializer(), \
