...
class AE_Supervised_Actor_Network(BaseNetwork):
    def __init__(self, sess, input_norm, layer_dim, state_dim, state_min, state_max, action_dim, action_min, action_max,
                 actor_lr, tau, num_modal, action_selection, norm_type):
        super(AE_Supervised_Actor_Network, self).__init__(sess, state_dim, action_dim, actor_lr, tau)
        ...
        self.inputs, self.phase, self.action, self.action_prediction_mean, self.action_prediction_sigma, self.action_prediction_alpha = self.build_network(
            scope_name='supervised_actor')
        self.net_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='supervised_actor')
        ...
        self.target_inputs, self.target_phase, self.target_action, self.target_action_prediction_mean, self.target_action_prediction_sigma, self.target_action_prediction_alpha = self.build_network(
            scope_name='target_supervised_actor')
        self.target_net_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope='target_supervised_actor')
    ...
    def build_network(self, scope_name):
        with tf.variable_scope(scope_name):
            ...
            action_prediction_mean, action_prediction_sigma, action_prediction_alpha = self.network(inputs, action, phase)
    ...
    def network(self, inputs, action, phase):
        # shared net
        action_net = tf.contrib.layers.fully_connected(inputs, self.l1, activation_fn=None,
        ...
        # action branch
        action_net = tf.contrib.layers.fully_connected(action_net, self.l2, activation_fn=None,
                                                       weights_initializer=tf.contrib.layers.variance_scaling_initializer(
                                                           factor=1.0, mode="FAN_IN", uniform=True),
                                                       # tf.truncated_normal_initializer(),
