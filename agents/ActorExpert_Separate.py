...
from agents.network import ae_actor_network
from agents.network import ae_expert_network
from experiment import write_summary
import utils.plot_utils


class ActorExpert_Separate_Network_Manager(BaseNetwork_Manager):
    def __init__(self, config):
        super(ActorExpert_Separate_Network_Manager, self).__init__(config)
        ...
        with self.graph.as_default():
            ...
            self.actor_network = ae_actor_network.AE_Actor_Network(self.sess, self.input_norm, config)
            self.expert_network = ae_expert_network.AE_Expert_Network(self.sess, self.input_norm, config)
...
class ActorExpert_Separate(BaseAgent):
    def __init__(self, config):

        network_manager = ActorExpert_Separate_Network_Manager(config)
        super(ActorExpert_Separate, self).__init__(config, network_manager)
